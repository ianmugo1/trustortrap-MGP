export function NextStepCard({ title, description, cta, onClick }) {
  return (
    <div className="rounded-2xl border border-dashed border-emerald-400/40 bg-gradient-to-br from-emerald-500/10 via-cyan-500/10 to-slate-900 px-4 py-4 text-xs text-slate-100">
      ...
      <button
        type="button"
        onClick={onClick}
        className="mt-3 inline-flex items-center rounded-full bg-emerald-500 px-3 py-1 text-[11px] font-semibold text-slate-950 hover:bg-emerald-400"
      >
        {cta}
      </button>
    </div>
